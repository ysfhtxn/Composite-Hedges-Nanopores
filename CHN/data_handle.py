...
def read_bits_from_file(path, segment_length=120, need_logs=True, rb=False):
    monitor = Monitor()
    if need_logs:
    ...
    padding_size = -len(values_bytes) % (segment_length//8)
    padding = bytes(padding_size)
    values_bytes = values_bytes + padding
    values_bytes_chunk = [values_bytes[i:i + segment_length//8] for i in range(0, len(values_bytes), segment_length//8)]
    if rb:
        return values_bytes_chunk, len(values)

...
from CHN.default import AbstractCodingAlgorithm, AbstractErrorCorrectionCode
from Chamaeleo.utils import indexer
from CHN import data_handle
...
class TranscodePipeline(DefaultPipeline):
    ...
    def __init__(self, **info):
        ...
        self.rb = info["rb"] if "rb" in info else False
    ...
    def transcode(self, **info):
        if "direction" in info:
            if info["direction"] == "t_c":
                ...
                rb = info["rb"] if "rb" in info else False
            ...
            for correct_name, error_correction in self.error_corrections.items():
                for file_name, file_path in self.file_paths.items():
                    ...
                    encoded_data = pipeline.transcode(direction="t_c", input_path=file_path,
                                                      ...
                                                      index=needed_index, index_length=self.index_length, rb=self.rb)

                    pipeline_logs = []
                    for iteration in range(self.iterations):
                        chosen_count = int(len(encoded_data["dna"]) * (1 - self.sequence_loss))
                        dna_sequences = random.sample(copy.deepcopy(encoded_data["dna"]), chosen_count) if chosen_count < len(encoded_data["dna"]) else encoded_data["dna"]

                        dna_sequences_array = numpy.array(dna_sequences)    

                        dna_sequences_array = dna_sequences_array.reshape(dna_sequences_array.shape[0], 1 ,dna_sequences_array.shape[1]) if dna_sequences_array.ndim == 2 else dna_sequences_array
                        ...
                        dna_sequences = []
                        for dna_sequence_array in dna_sequences_array:
                            
                            error_array = numpy.zeros_like(dna_sequence_array, dtype=object)
                            snv_num = int(dna_sequence_array.size * self.nucleotide_mutation)
                            del_num = int(dna_sequence_array.size * self.nucleotide_deletion)
                            ins_num = int(dna_sequence_array.size * self.nucleotide_insertion)
                            # randomly select the positions and store the error type in the error array
                            # snv error
                            indices_with_errors = numpy.argwhere(error_array == 0)
                            snv_indices = indices_with_errors[numpy.random.choice(len(indices_with_errors), snv_num, replace=False)]
                            ...
                            # del error
                            indices_with_errors = numpy.argwhere(error_array == 0)
                            del_indices = indices_with_errors[numpy.random.choice(len(indices_with_errors), del_num, replace=False)]
                            ...
                            # ins error
                            indices_with_errors = numpy.argwhere(error_array == 0)
                            ins_indices = indices_with_errors[numpy.random.choice(len(indices_with_errors), ins_num, replace=False)]
                            ...
                            errors_seqs = copy.deepcopy(dna_sequence_array)
                            bases = ['A', 'C', 'G', 'T']
                            for row, col in list(product(range(cols), range(rows))):
                                if error_array[row, col] == 'snv':
                                    current_base = dna_sequence_array[row, col]
                                    chosen_bases = [base for base in bases if base != current_base]
                                    errors_seqs[row, col] = numpy.random.choice(chosen_bases)
                                elif error_array[row, col] == 'del':
                                    errors_seqs[row, col] = ''
                                elif error_array[row, col] == 'ins':
                                    errors_seqs[row, col] = numpy.random.choice(bases) + dna_sequence_array[row, col]

                            seqs_strings = [list(''.join(seq)) for seq in errors_seqs]

                            dna_sequences.append(seqs_strings)

                        dna_sequences = [item[0] for item in dna_sequences] if len(dna_sequences[0]) == 1 else dna_sequences

...
class AbstractCodingAlgorithm(object):
    ...
    def silicon_to_carbon(self, bit_segments, bit_size):
        for bit_segment in bit_segments:
            ...
            if (type(bit_segment) != list and type(bit_segment) != bytes) or type(bit_segment[0]) != int :
    ...
    def carbon_to_silicon(self, dna_sequences):
        ...
        for dna_sequence in dna_sequences:
            if type(dna_sequence) != list or (type(dna_sequence[0]) != str and type(dna_sequence[0]) != list):
